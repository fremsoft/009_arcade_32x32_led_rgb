...
/* sistema bicolor: 
 *  nei tempi pari usa RGB del nibble basso,
 *  nei tempi dispari usa RGB del nibble alto
 */
uint8_t ledmap[32][32]/*[3]*/;
uint8_t alternanza_nibble;
...
void displayDrawLine(uint8_t line) {
  ...
  if (alternanza_nibble == 0) {  
      // caricamento bit riga pari
      for (int i = 0; i < 32; i++) {
      ...
      // caricamento bit riga dispari
      for (int i = 0; i < 32; i++) {
          digitalWrite(R1, ledmap[i][line]&0x10/*[0]*/);
          digitalWrite(G1, ledmap[i][line]&0x20/*[1]*/);
          digitalWrite(B1, ledmap[i][line]&0x40/*[2]*/);
    
          digitalWrite(R2, ledmap[i][line+16]&0x10/*[0]*/);
          digitalWrite(G2, ledmap[i][line+16]&0x20/*[1]*/);
          digitalWrite(B2, ledmap[i][line+16]&0x40/*[2]*/);
...
void displayDrawBackground(int bg) {
  ...
  if (bg == 1) {
    for (i=0; i<32; i++) {
      for (j=0; j<32; j++) {
        col = pgm_read_byte(&move_to_play[i][j]);
    ...
    for (i=0; i<32; i++) {
      for (j=0; j<32; j++) {
        col = pgm_read_byte(&game_over[i][j]);
        if ((col & 0xF0) == 0) { col = (col << 4) | col; } 
